...
div(class="time-picker-modal-container")
	div(class="time-picker-modal-header")
		span(@click="handleStepChange(0)" class="time-picker-header" ':class'="{active:!step}") {{hourString > 12 ? String(hourString-12).padStart(2, '0') : hourString}}
		| &nbsp;:&nbsp;
...
export default {
  ...
  methods: {
    ...
    resetMinuteDegree() {
      ...
      let pointerR = 0
      TIME_ARRAY.MINUTES.forEach((minute, index) => {
        if (m === index) {
